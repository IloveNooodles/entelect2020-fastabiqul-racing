...
public class Bot {
  ...
  public Command run(GameState gameState) {
    ...
    if (endGame) {
      ...
      if (containBoost(blocksFront) && canForward == 0) {
        Command useOilTweetEndgame = USE_OIL_TWEET_ENDGAME(hasOil,  hasTweet, opponentBlock, myCarBlock, opponentLane, myCarLane, canLeft, canRight, opponentSpeed)
    ...
    if (containBoost(blocksFront)) {
      Command desicion =  PowerupDesicion(canForward, canLeft, canRight, canAccelerate, opponentLane, myCarLane, opponentBlock, myCarBlock, opponentSpeed, countOil, countTweet, countLizard, hasLizard, hasOil, hasTweet, lastBlocked);
      if(desicion != null){
        return desicion;
      }
      // if (canForward == 0) {
    ...
    //   return DECELERATE;
    // }
    Command PrioPowerUpFarLand = prioPowerUpFarLand( blocksRightFar,  blocksLeftFar,  canForwardFar,  canRightFar,  canLeftFar,  myCarSpeed,  myCarLane,  myCarBlock,  opponent, gameState);
  ...
  private Command prioPowerUpFarLand(int blocksRightFar, int blocksLeftFar, int canForwardFar, int canRightFar, int canLeftFar, int myCarSpeed, int myCarLane, int myCarBlock, int opponent, GameState gameState){
    if (containBoost(blocksRightFar) && canRightFar <= canForwardFar && canRightFar <= canLeftFar) {
      return TURN_RIGHT;
    }
    if (containBoost(blocksLeftFar) && canLeftFar <= canForwardFar && canLeftFar <= canRightFar) {
  ...
  private Command USE_OIL_TWEET(int countOil, int countTweet, int myCarLane, int opponentLane, int opponentBlock, int opponentSpeed){
    Command TWEET;
    if (countOil > 3)
      return OIL;
    if (countTweet > 3 && myCarLane != opponentLane) {
  ...
  private Command USE_OIL_TWEET_ENDGAME(int hasOil, int, hasTweet, int opponentBlock, int myCarBlock, int opponentLane, int myCarLane, int canLeft, int canRight, int opponentSpeed){
    Command TWEET;
    if ((hasOil) && (Math.abs(opponentLane - myCarLane) <= 1)
            && (opponentBlock <= myCarBlock)
            && ((opponentLane == myCarLane) || (canLeft != 0 && canRight != 0))) {
